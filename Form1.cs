...
namespace CaesarProject
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeCustomComponents()
        {
            ...
            this.AllowDrop = true; //не работает почему то
            ...
            // СОБЫТИЯ
            // С Drag and Drop помогла нейронка
            ...
            // Обработка ошибок тоже была подсмотрена у AI, а так все были сделаны по референсу, с пониманием конечно.
            btnEncrypt.Click += (s, e) => {
                ...
                try {
                    if (txtOutput != null && numShift != null)
                        txtOutput.Text = ApplyCaesar(txtInput.Text, (int)numShift.Value);
                }
        ...
        private string ApplyCaesar(string input, int shift) {
            ...
            foreach (char c in input) {
                if (c >= 'a' && c <= 'z') res.Append((char)('a' + (c - 'a' + (shift % 26) + 26) % 26));
                    ...
                    throw new FormatException($"Обнаружен неподдерживаемый символ: '{c}'. Разрешены только символы латиницы, кириллицы, цифры и знаки препинания.");
            }
            return res.ToString();
        }
    }
